...
jQuery(function() {
	$('.ticket').click(function() {
		setTimeout(function(){
			window.open("http://airticket.traicy.com/");
		}, 1000);
	});
});
jQuery(function() {
	$('.rentACar').click(function() {
		setTimeout(function(){
			window.open("http://h.accesstrade.net/sp/cc?rk=0100kmfy00ada9");
		}, 1000);
	});
});
</script>
			...
			<div id="variationCompe">
				<ul id="variationCompeTab">
					<li class="select">Wifi価格</li>
					<li class="ticket"> 航空券 </li>
					<li class="rentACar"> レンタカー </li>
					<!-- <li> <?php //旅行保険 ?> </li> -->
				</ul>
                <div class="contentCompe">
                    <?php global $isCompeBox;?>
                    <?php $isCompeBox = true;?>
					<div class="show"><?php get_template_part('form'); ?></div>
                    <?php $isCompeBox = false; ?>
					<div class="show articleHide ticket">別ページに遷移します．</div>
					<div class="show articleHide rentACar">別ページに遷移します．</div>
					<!-- <div class="show articleHide"></div> -->
				</div>
			</div>
		</div> <!-- topComepe -->
	</div> <!-- compeBox -->
